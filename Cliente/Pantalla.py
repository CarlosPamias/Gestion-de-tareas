...
class Product:
    def __init__(self, window):
        super().__init__(window,width=600, height=600)
        self.wind = window
        self.wind.title = "Gestion de proyectos"
        self.pack()
...
#Creamos el Frame 1 para intorducir los datos y y el frame 2 para seleccionar los datos de la base de datos
        self.frame_uno = Frame(bg='white', height=200, width=800)
        self.frame_uno.grid(column=0, row=0, sticky='nsew')
        self.frame_dos = Frame(bg='white', height=300, width=800)
        ...
        for fila in datos:
            ...
            if tematica_db == tematica:
                if codigo != None:
                    ...
                    fecha_fin =self.fecha_fin.get()
                    if tematica and academia and prioridad and descripcion != '':
                        self.Bd.modificar_tarea(codigo,tematica,prioridad,descripcion,fecha_inicio,fecha_fin)
                        self.tabla.delete(*self.tabla.get_children())
                        datos = self.Bd.obtener_todas_las_tareas()
                        i = -1
                        for dato in datos:
