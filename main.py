...
class Ventana(Frame):
    def __init__(self, master):
        ...
        self.fecha_fin_prevista = StringVar()
...
#creamos los botones para ejecutar acciones
        ...
        Button(self.frame_uno,text= 'Modificar', font=('arial', 9, 'bold'),bg='white',
               width=20,bd=3,command = self.agregardatos).grid (column=2, row=3, pady=5,padx=5)
...
#Creamos un estilo de tabla para mostrar la información
    ...
    def elininar_datos(self, event):
        self.limpiar_datos()
        item = self.tabla.selection()[0]
        x = messagebox.askquestion('Informacion', '¿Desea eliminar?')
        if x == 'yes':
            self.tabla.delete(item)
            self.base_datos.eliminar_tarea(self.data['text'])
    ...
    def agregar_datos(self):
        ...
        if tematica and academia and prioridad and descripcion != ' ':
            ...
            self.base_datos.agregar_tarea(tematica, academia,prioridad,fecha_inicio,fecha_fin)
            self.limpiar_campos()
        
    def actualizar_tabla(self):
        self.limpiar_campos()
        datos = self.base_datos.obtener_todas_las_tareas()
        ...
        for fila in datos:
            ...
            if tematica_db == tematica:
                if codigo != None:
                    ...
                    fecha_fin =self.fecha_fin.get()
                    if tematica and academia and prioridad and descripcion != '':
                        self.base_datos.modificar_tarea(codigo,tematica,prioridad,descripcion,fecha_inicio,fecha_fin)
                        self.tabla.delete(*self.tabla.get_children())
                        datos = self.base_datos.obtener_todas_las_tareas()
                        i = -1
                        for dato in datos:
